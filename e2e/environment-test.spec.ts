...
test.describe('Environment Variables Test Component', () => {
  ...
  test('should render environment test component correctly', async ({ page }) => {
  ...
  test('should work within the homepage context', async ({ page, browserName }) => {
    await envTestPage.goto();
    ...
    await TestUtils.takeScreenshot(page, `environment-component-${browserName}`);
  });
  ...
  test('should handle responsive layout within homepage', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      await TestUtils.takeScreenshot(page, `environment-component-responsive-${viewport.name}`);
    }
  });
});
